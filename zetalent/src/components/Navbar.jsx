import Link from "next/link";
import Image from "next/image";
import React, { useEffect, useState } from "react";
import ZetaLogo from "./ZetaLogo";
import { useStateProvider } from "@/context/StateContext";
import { IoSearchOutline } from "react-icons/io5";
import { usePathname, useRouter } from "next/navigation";
import { useCookies } from "react-cookie";
import axios from "axios";
import { GET_USER_INFO, HOST } from "@/utils/constants";
import { reducerCases } from "@/context/constants";


const Navbar = () => {
  const router = useRouter();
  const pathname = usePathname();
  const [cookies] = useCookies();
  const [isLoaded, setIsLoaded] = useState(false);
  const [isFixed, setIsFixed] = useState(false);
  const [searchData, setSearchData] = useState("");
  const [isProfileMenuVisible, setIsProfileMenuVisible] = useState(false);
  const [{ showLoginModule, showSignupModule, userInfo, isSeller }, dispatch] =
    useStateProvider();

  useEffect(() => {
    if (pathname === "/") {
      setIsFixed(false);

      const positionNavbar = () => {
        setIsFixed(window.pageYOffset > 0);
      };

      window.addEventListener("scroll", positionNavbar);

      return () => {
        window.removeEventListener("scroll", positionNavbar);
      };
    } else {
      setIsFixed(true);
    }
  }, [pathname]);

    useEffect(() => {
    if (cookies.jwt && !userInfo) {
      const getUserInfo = async () => {
        try {
          const {
            data: { user },
          } = await axios.post(GET_USER_INFO, {}, { 
            headers:{
              Authorization: `Bearer ${cookies.jwt}`
            }
           });
          let projectedUserInfo = { ...user };
          if (user.profileImage) {
            projectedUserInfo = {
              ...projectedUserInfo,
              imageName: HOST + "/" + user.profileImage,
            };
          }
          delete projectedUserInfo.image;
          dispatch({
            type: reducerCases.SET_USER,
            userInfo: projectedUserInfo,
          });
          setIsLoaded(true);
          if (user.isProfileInfoSet === false && pathname !== "/profile") {
            router.push("/profile")
          }
        } catch (err) {
          console.log(err);
        }
      };
      getUserInfo();
    } else {
      setIsLoaded(true);
    }
  }, [cookies, userInfo]);

  const handleLogin = () => {
    if (showSignupModule) {
      dispatch({
        type: reducerCases.TOGGLE_SIGNUP_MODULE,
        showSignupModule: false,
      });
    }
    dispatch({
      type: reducerCases.TOGGLE_LOGIN_MODULE,
      showLoginModule: true,
    });
  };

  const handleSignup = () => {
    if (showLoginModule) {
      dispatch({
        type: reducerCases.TOGGLE_LOGIN_MODULE,
        showLoginModule: false,
      });
    }
    dispatch({
      type: reducerCases.TOGGLE_SIGNUP_MODULE,
      showSignupModule: true,
    });
  };

  const handleOrdersNavigate = () => {
    if(isSeller) 
      {router.push("/seller/orders")}
    else{
    router.push("/buyer/orders");}
  };

  const handleModeSwitch = () => {
    const newPath = isSeller ? "/buyer/orders" : "/seller";
    dispatch({ type: reducerCases.SWITCH_MODE });
    router.push(newPath);
  };

  const links = [
    { linkName: "Zetalent Business", handler: "#", type: "link" },
    { linkName: "Explore", handler: "#", type: "link" },
    { linkName: "English", handler: "#", type: "link" },
    { linkName: "Become a Seller", handler: "#", type: "link" },
    { linkName: "Sign in", handler: handleLogin, type: "button" },
    { linkName: "Join", handler: handleSignup, type: "button2" },
  ];

  return (
    <>
      {isLoaded && (
        <nav
          className={`w-full px-24 flex justify-between items-center py-6 top-0 z-30 transition-all duration-300 
      ${
        isFixed || userInfo
          ? "fixed bg-white border-b border-gray-200"
          : "absolute bg-transparent border-transparent"
      }`}
        >
          <div>
            <Link href="/">
              <ZetaLogo
                fillColor={!isFixed && !userInfo ? "#ffffff" : "#404145"}
              />
            </Link>
          </div>
          <div
            className={`flex ${
              isFixed || userInfo ? "opacity-100" : "opacity-0"
            }`}
          >
            <input
              type="text"
              className="w-[30rem] py-2.5 px-4 border text-[#6b6b6b]"
              value={searchData}
              onChange={(e) => setSearchData(e.target.value)}
              placeholder={`Type the service you're looking for`}
            />
            <button
              className="bg-gray-900 py-1.5 text-white w-16 flex justify-center items-center"
              onClick={() => {
                setSearchData("");
                router.push(`/search?q=${searchData}`);
              }}
            >
              <IoSearchOutline className="fill-white text-white h-6 w-6" />
            </button>
          </div>
          {!userInfo ? (
            <ul className="flex gap-10 items-center ">
              {links.map(({ linkName, handler, type }) => {
                return (
                  <li
                    key={linkName}
                    className={`${
                      isFixed ? "text-[#515151]" : "text-white"
                    } font-medium`}
                  >
                    {type === "link" && <Link href={handler}>{linkName}</Link>}
                    {type === "button" && (
                      <button onClick={handler}>{linkName}</button>
                    )}
                    {type === "button2" && (
                      <button
                        onClick={handler}
                        className={`border text-md font-semibold py-1 px-3 rounded-sm
                 ${
                   isFixed
                     ? "border-[#1DBF73] text-[#1DBF73]"
                     : "border-white text-white "
                 }
              hover:bg-[#1DBF73] hover:text-white hover:border-[#1dbf73] transition-all duration-500 `}
                      >
                        {linkName}
                      </button>
                    )}
                  </li>
                );
              })}
            </ul>
          ) : (
            <ul className="flex gap-10 items-center">
              {isSeller && (
                <li
                  className="cursor-pointer text-[#1DBF73] font-medium"
                  onClick={() => router.push("/seller/gigs/create")}
                >
                  Create Gig
                </li>
              )}
              <li
                className="cursor-pointer text-[#1DBF73] font-medium"
                onClick={handleOrdersNavigate}
              >
                Orders
              </li>
              <li
                className="cursor-pointer font-medium text-[#565656]"
                onClick={handleModeSwitch}
              >
                Switch to {isSeller ? "Buyer" : "Seller"}
              </li>

              <li
                className="cursor-pointer"
                onClick={(e) => {
                  e.stopPropagation();
                  //setIsContextMenuVisible(true);
                }}
                title="Profile"
              >
                {userInfo?.imageName ? (
                  <Image
                    src={userInfo.imageName}
                    alt="Profile"
                    width={40}
                    height={40}
                    className="rounded-full"
                  />
                ): (
                  <div className="bg-purple-500 h-10 w-10 flex justify-center items-center rounded-full relative">
                    <span className="text-xl text-white">
                      {userInfo?.email?.[0]?.toUpperCase()}
                    </span>
                  </div>
                )}
              </li>
            </ul>
          )}
        </nav>
      )}{" "}
    </>
  );
};

export default Navbar;
